...
def block1(x, filters, next_filter, kernel_size=3, stride=1, conv_shortcut=True, name=None, sp_rate=1e-5, thr_init=1e-1): # next_filter to calculate n_outputs 
    ...
    x, s, n = delta_activation(sp_rate=sp_rate, thr_init=thr_init, name=name + '_1_delta', n_outputs=filters*kernel_size*kernel_size, show_metric=show_metric, thr_trainable=thr_trainable)(x) #<--
    n_spikes += s
    n_neurons +=n
    ...
    x, s, n = delta_activation(sp_rate=sp_rate, thr_init=thr_init, name=name + '_2_delta', n_outputs=4*filters*1*1, show_metric=show_metric, thr_trainable=thr_trainable)(x) #<--
    n_spikes += s
    n_neurons +=n
    ...
    x, s, n = delta_activation(sp_rate=sp_rate, thr_init=thr_init, name=name + '_out_delta', n_outputs=next_filter, show_metric=show_metric, thr_trainable=thr_trainable)(x) #<--
    n_spikes += s
    n_neurons +=n
...
def UCF_ResNet50_delta(input_shape=(224,224), classes=101, sp_rate=1e-5, thr_init=None, thr_trainable=True):

    img_input = layers.Input(shape=(None, input_shape[0], input_shape[1],3))
    x, s_in, n_in = delta_activation(sp_rate=sp_rate, thr_init=thr_init[0], name='inp_delta', n_outputs=64*7*7, show_metric=show_metric, thr_trainable=thr_trainable)(img_input) #<--
    ...
    x, s_conv1, n_conv1 = delta_activation(sp_rate=sp_rate, thr_init=thr_init[1], name='conv1_delta', n_outputs=64*5*1*1, show_metric=show_metric, thr_trainable=thr_trainable)(x) #<--
    ...
    x, s_fc, n_fc = delta_activation(sp_rate=sp_rate, thr_init=thr_init[6], name='fc'+str(1024)+'_delta', n_outputs=classes, show_metric=show_metric, thr_trainable=thr_trainable)(x) #<--
    ...
    avg = tf.math.reduce_mean(predictions, axis=1) #average predictions over time #predictions[:,0,:] #-->for single frames #
    # Create model.
